...
(** Deprecated tree-local focused decomposition.  The supported access
    normalization path now lives in [typed_conditional_raven_adapter] and is
    exposed by [AlignedNormalization.aligned_suffix_has_valid_trace].  Keep
    this implementation namespaced temporarily for comparison while clients
    migrate; it is deliberately no longer part of the active runtime API. *)
Module LegacyFocusedAccess.

(** A certificate [focused_certificate_core] closes the access marker at the
    head of the stack.  It records the tree-shaped location of the matching
    fold.  In particular, a conditional has to close the marker in *both*
    branches: there is no single linear certificate suffix common to its two
    executions.

    This judgment deliberately mentions only certificates and their exact
    LIFO indices.  Hoare-rule wrappers (frame, consequence, and existential
    rules) do not alter the certificate; [focused_core] below pairs this
    structural witness with the possibly wrapped aligned derivation. *)
Inductive focused_certificate_core
    (cost : GenericRegions.Atomicity.cost_model) Γ
    (invariant : inv_id) (outer_open : gset inv_id)
    (tail : list GenericRegions.Atomicity.access_marker) :
    forall fuel entry statement exit,
      GenericRegions.Atomicity.analysis_certificate
        cost Γ fuel entry statement exit -> Type :=
| FocusedCertificateFold fuel entry node arguments
    (view : RegionSyntax.view (TFold node invariant arguments) =
      TypedAnalysisView.ViewFold invariant)
    (certificate := @GenericRegions.Atomicity.CertFold
      cost Γ fuel entry (TFold node invariant arguments) invariant view) :
    GenericRegions.Atomicity.lifo_certificate certificate
      ((invariant, outer_open) :: tail) tail ->
    focused_certificate_core cost Γ invariant outer_open tail
      (S fuel) entry (TFold node invariant arguments)
      (GenericRegions.Atomicity.fold_invariant invariant entry) certificate
| FocusedCertificateSequenceLeft fuel entry node first middle second exit
    (first_certificate : GenericRegions.Atomicity.analysis_certificate
      cost Γ fuel entry first middle)
    (second_certificate : GenericRegions.Atomicity.analysis_certificate
      cost Γ fuel middle second exit)
    (view : RegionSyntax.view (TSeq node first second) =
      TypedAnalysisView.ViewSequence first second) :
    focused_certificate_core cost Γ invariant outer_open tail
      fuel entry first middle first_certificate ->
    GenericRegions.Atomicity.lifo_certificate second_certificate tail tail ->
    focused_certificate_core cost Γ invariant outer_open tail (S fuel) entry
      (TSeq node first second) exit
      (@GenericRegions.Atomicity.CertSequence cost Γ fuel entry
        (TSeq node first second) first middle second exit
        view first_certificate second_certificate)
| FocusedCertificateSequenceRight fuel entry node first middle second exit
    (first_certificate : GenericRegions.Atomicity.analysis_certificate
      cost Γ fuel entry first middle)
    (second_certificate : GenericRegions.Atomicity.analysis_certificate
      cost Γ fuel middle second exit)
    (view : RegionSyntax.view (TSeq node first second) =
      TypedAnalysisView.ViewSequence first second) :
    GenericRegions.Atomicity.lifo_certificate first_certificate
      ((invariant, outer_open) :: tail)
      ((invariant, outer_open) :: tail) ->
    focused_certificate_core cost Γ invariant outer_open tail
      fuel middle second exit second_certificate ->
    focused_certificate_core cost Γ invariant outer_open tail (S fuel) entry
      (TSeq node first second) exit
      (@GenericRegions.Atomicity.CertSequence cost Γ fuel entry
        (TSeq node first second) first middle second exit
        view first_certificate second_certificate)
| FocusedCertificateConditional fuel entry node condition
    then_branch else_branch then_exit else_exit
    (then_certificate : GenericRegions.Atomicity.analysis_certificate
      cost Γ fuel entry then_branch then_exit)
    (else_certificate : GenericRegions.Atomicity.analysis_certificate
      cost Γ fuel entry else_branch else_exit)
    ...
    (view : RegionSyntax.view
      (TIf node condition then_branch else_branch) =
      TypedAnalysisView.ViewConditional then_branch else_branch) :
    focused_certificate_core cost Γ invariant outer_open tail
      fuel entry then_branch then_exit then_certificate ->
    focused_certificate_core cost Γ invariant outer_open tail
      fuel entry else_branch else_exit else_certificate ->
    focused_certificate_core cost Γ invariant outer_open tail (S fuel) entry
      (TIf node condition then_branch else_branch)
      (GenericRegions.Atomicity.AnalysisState
        (GenericRegions.Atomicity.analysis_mask then_exit ∩
        ...
        (GenericRegions.Atomicity.analysis_step_taken then_exit ||
          GenericRegions.Atomicity.analysis_step_taken else_exit)
        (GenericRegions.Atomicity.analysis_in_atomic then_exit))
      (@GenericRegions.Atomicity.CertConditional cost Γ fuel entry
...
(** The aligned version retains the assertion indices and hence all
    assertion-only wrappers around the control-producing rule. *)
Record focused_core
    (cost : GenericRegions.Atomicity.cost_model) Γ F Δ
    (invariant : inv_id) (outer_open : gset inv_id)
    (tail : list GenericRegions.Atomicity.access_marker)
    ...
    (derivation : @Certified.Rules.RavenHoareTriple Γ F Δ pre statement
      (GenericRegions.Atomicity.analysis_mask entry)
      (GenericRegions.Atomicity.analysis_mask exit) post) : Type := {
  focused_core_aligned :
    Certified.certificate_hoare_aligned cost certificate derivation;
  focused_core_lifo : GenericRegions.Atomicity.lifo_certificate certificate
    ((invariant, outer_open) :: tail) tail;
  focused_core_shape : focused_certificate_core cost Γ invariant outer_open
    tail fuel entry statement exit certificate;
}.

(** Returning the same stack is not enough for a chunk before the matching
    close: such a chunk could close and then reopen the focused invariant.
    This structural witness says that no path through the certificate folds
    the focused invariant.  Assertion-only Hoare wrappers are transparent
    because they retain the same certificate. *)
Fixpoint focused_preserving_certificate
    {cost Γ fuel entry statement exit}
    (focused : inv_id)
    (certificate : GenericRegions.Atomicity.analysis_certificate
      cost Γ fuel entry statement exit) : Prop :=
  match certificate with
  | GenericRegions.Atomicity.CertLeaf _ _ _ _ _ _ _ _ => True
  | GenericRegions.Atomicity.CertUnfold _ _ _ _ _ _ _ _ _ => True
  | GenericRegions.Atomicity.CertFold _ _ _ _ _ folded _ =>
      folded = focused -> False
  | GenericRegions.Atomicity.CertSequence _ _ _ _ _ _ _ _ _ _
      first_certificate second_certificate =>
      focused_preserving_certificate focused first_certificate /\
      focused_preserving_certificate focused second_certificate
  | GenericRegions.Atomicity.CertConditional _ _ _ _ _ _ _ _ _ _
      then_certificate else_certificate _ _ =>
      focused_preserving_certificate focused then_certificate /\
      focused_preserving_certificate focused else_certificate
  | GenericRegions.Atomicity.CertAtomic _ _ _ _ _ _ _ _ _ _
      body_certificate _ =>
      focused_preserving_certificate focused body_certificate
  end.

(** A focused prefix skips zero or more chunks which preserve the focused
    stack, then exposes the tree-shaped closing core.  The remaining suffix
    is retained verbatim after the close. *)
Inductive focused_prefix
    (cost : GenericRegions.Atomicity.cost_model) Γ F Δ
    (invariant : inv_id) (outer_open : gset inv_id)
    (tail : list GenericRegions.Atomicity.access_marker) :
    forall entry (pre : assertion Γ F Δ) exit (post : assertion Γ F Δ)
      stack_out,
      aligned_operational_suffix cost Γ F Δ entry pre
        ((invariant, outer_open) :: tail) exit post stack_out -> Type :=
| FocusedPrefixHere fuel entry statement middle pre middle_assertion
    (certificate : GenericRegions.Atomicity.analysis_certificate
      cost Γ fuel entry statement middle)
    (derivation : @Certified.Rules.RavenHoareTriple Γ F Δ pre statement
      (GenericRegions.Atomicity.analysis_mask entry)
    ...
    (lifo : GenericRegions.Atomicity.lifo_certificate certificate
      ((invariant, outer_open) :: tail) tail)
    (core : focused_certificate_core cost Γ invariant outer_open tail
      fuel entry statement middle certificate)
    final post stack_out
    (rest : aligned_operational_suffix cost Γ F Δ middle middle_assertion
      tail final post stack_out) :
    focused_prefix cost Γ F Δ invariant outer_open tail entry pre final post
      stack_out
      (@AlignedOperationalCons cost Γ F Δ fuel entry statement middle pre
        middle_assertion ((invariant, outer_open) :: tail) tail certificate
        derivation aligned lifo final post stack_out rest)
| FocusedPrefixLater fuel entry statement middle pre middle_assertion
    (certificate : GenericRegions.Atomicity.analysis_certificate
      cost Γ fuel entry statement middle)
    (derivation : @Certified.Rules.RavenHoareTriple Γ F Δ pre statement
      (GenericRegions.Atomicity.analysis_mask entry)
    ...
    (lifo : GenericRegions.Atomicity.lifo_certificate certificate
      ((invariant, outer_open) :: tail)
      ((invariant, outer_open) :: tail))
    (preserving : focused_preserving_certificate invariant certificate)
    final post stack_out
    (rest : aligned_operational_suffix cost Γ F Δ middle middle_assertion
      ((invariant, outer_open) :: tail) final post stack_out) :
    focused_prefix cost Γ F Δ invariant outer_open tail middle
      middle_assertion final post stack_out rest ->
    focused_prefix cost Γ F Δ invariant outer_open tail entry pre final post
      stack_out
      (@AlignedOperationalCons cost Γ F Δ fuel entry statement middle pre
        middle_assertion ((invariant, outer_open) :: tail)
        ((invariant, outer_open) :: tail) certificate derivation aligned lifo
        final post stack_out rest).

Lemma focused_certificate_core_lifo
    {cost Γ invariant outer_open tail fuel entry statement exit certificate}
    (core : focused_certificate_core cost Γ invariant outer_open tail
      fuel entry statement exit certificate) :
  GenericRegions.Atomicity.lifo_certificate certificate
    ((invariant, outer_open) :: tail) tail.
Proof.
  induction core; simpl.
  - exact l.
  - eexists. split; [exact IHcore|exact l].
  - eexists. split; [exact l|exact IHcore].
  - split; assumption.
Qed.

Definition focused_core_of_shape
    {cost Γ F Δ invariant outer_open tail fuel entry statement exit pre post}
    {certificate : GenericRegions.Atomicity.analysis_certificate
      cost Γ fuel entry statement exit}
    {derivation : @Certified.Rules.RavenHoareTriple Γ F Δ pre statement
      (GenericRegions.Atomicity.analysis_mask entry)
      (GenericRegions.Atomicity.analysis_mask exit) post}
    (aligned : Certified.certificate_hoare_aligned cost certificate derivation)
    (shape : focused_certificate_core cost Γ invariant outer_open tail
      fuel entry statement exit certificate) :
    focused_core cost Γ F Δ invariant outer_open tail fuel entry statement
      exit pre post certificate derivation :=
  {| focused_core_aligned := aligned;
     focused_core_lifo := focused_certificate_core_lifo shape;
     focused_core_shape := shape |}.

Lemma focused_prefix_erased_lifo
    {cost Γ F Δ invariant outer_open tail entry pre exit post stack_out}
    {suffix : aligned_operational_suffix cost Γ F Δ entry pre
      ((invariant, outer_open) :: tail) exit post stack_out}
    (focus : focused_prefix cost Γ F Δ invariant outer_open tail entry pre exit
      post stack_out suffix) :
  operational_suffix_lifo (erase_aligned_operational_suffix suffix)
    ((invariant, outer_open) :: tail) stack_out.
Proof. apply erase_aligned_operational_suffix_lifo. Qed.

Lemma focused_prefix_nonempty
    {cost Γ F Δ invariant outer_open tail entry pre exit post stack_out}
    {suffix : aligned_operational_suffix cost Γ F Δ entry pre
      ((invariant, outer_open) :: tail) exit post stack_out}
    (focus : focused_prefix cost Γ F Δ invariant outer_open tail entry pre exit
      post stack_out suffix) :
  exists fuel statement middle certificate rest,
    erase_aligned_operational_suffix suffix =
      @OperationalCons cost Γ fuel entry statement middle exit certificate rest.
Proof.
  destruct focus; eexists _, _, _, _, _; reflexivity.
Qed.

End LegacyFocusedAccess.
